...
def carregar_geojson_cdmun(path: Path):
    """Carrega o GeoJSON (em .json) e garante properties.CD_MUN como string de 7 dígitos."""
    with open(path, "r", encoding="utf-8") as f:
        gj = json.load(f)
    for ft in gj.get("features", []):
        p = ft.get("properties", {}) or {}
        cd = (p.get("CD_MUN") or p.get("CD_GEOCMU") or p.get("CD_GEOCODI") or
              p.get("CD_MUNIC") or p.get("CD_IBGE") or p.get("GEOCODIGO") or
              p.get("IBGE") or p.get("id"))
        cd = "".join(str(cd))
        cd = "".join(ch for ch in cd if ch.isdigit()).zfill(7)
        p["CD_MUN"] = cd
        ft["properties"] = p
    return gj

def lookup_nomes_from_geojson(gj):
    """Tenta recuperar nomes de município do GeoJSON (se existirem)."""
    name_keys = [
        "NM_MUN", "NM_MUNICIP", "NM_MUNICIPIO", "NM_MUNIC", "NM_NOME",
...
def calc_ehf(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["T3d_prev"] = (df.groupby("CD_MUN")["Tmean"].apply(_t3_centered).reset_index(level=0, drop=True))
...
def build_combined_risk(df: pd.DataFrame):
    if "GeoSES" not in df.columns:
        ...
        df["risk_class"] = "Normal"
        return df, False
    d = df.copy()
    geoses_num = pd.to_numeric(d["GeoSES"], errors="coerce")
    d["V"] = ((1 - geoses_num) / 2).clip(0, 1)  # 0 = baixa vulnerabilidade; 1 = alta
...
if not ARQ_PREV.exists():
    raise FileNotFoundError(f"Arquivo não encontrado: {ARQ_PREV}")
if not ARQ_ATTR.exists():
    raise FileNotFoundError(f"Arquivo não encontrado: {ARQ_ATTR}")
...
keep_attr = [
    "CD_MUN", "NM_MUN", "SIGLA_UF", "NM_REGIAO", "GeoSES",
    ...
    children=[
        ...
        html.Div(
            [
                ...
                html.Div(
                    [
                        dcc.Graph(id="mapa", style={"height": "58vh", "marginBottom": "8px"}),
                        html.Div(
                            id="cards-ehf",
                            style={
                                "display": "grid",
...
def cb_viz(idx_date, reg_key, uf_keys, muni_key, layer):
    ...
    vis["CD_MUN"] = vis["CD_MUN"].astype(str).str.extract(r"(\d+)")[0].str.zfill(7)
    vis["muni_label"] = vis["NM_MUN"].astype(str) + " / " + vis["SIGLA_UF"].astype(str)

    # Subconjunto do GeoJSON apenas com os municípios do dia (melhora perf)
    ids_vis = set(vis["CD_MUN"].dropna().unique().tolist())
    feats = [ft for ft in GJ.get("features", [])
             if str(ft.get("properties", {}).get("CD_MUN", "")).zfill(7) in ids_vis]
    GJ_SUB = {"type": "FeatureCollection", "features": feats or GJ.get("features", [])}

    # Log útil nos builds/render logs para depurar matching
    print("[MAP]", "vis rows:", len(vis),
          "| ids vis únicos:", vis["CD_MUN"].nunique(),
          "| feats GJ_SUB:", len(GJ_SUB.get("features", [])))

    # >>> Mapa sem Mapbox (mais robusto)
    ...
    serie = (
        dmun[["data", "Tmin", "Tmean", "Tmax", "NM_MUN", "SIGLA_UF", "classification"]]
        .dropna(subset=["data"])
        .sort_values("data")
    )
    ...
    bt = serie.melt(
        id_vars=["data", "data_lbl", "NM_MUN", "SIGLA_UF"],
        value_vars=["Tmin", "Tmean", "Tmax"],
        var_name="Série",
        value_name="Valor",
    )
...
def _df_export_full():
    ...
    out = df[list(cols.keys())].rename(columns=cols).sort_values(["Data", "UF", "NM_MUN"])
